...
const envVarsSchema = Joi.object({
	...
	SERVER_HOSTNAME: Joi.string()
		.required()
		.description("Localhost During Development"),
	...
	DB_NAME_DIGITAL_OCEAN: Joi.string()
		.default("reepls-db")
		.description("Digital Ocean database name"),
	DB_TIMEOUT_DIGITAL_OCEAN: Joi.number()
		.default(20000)
		.description(
			"Digital Ocean database connection timeout in milliseconds"
		),
	MAX_IDLE_TIME_DIGITAL_OCEAN: Joi.number()
		.default(10000)
		.description("Digital Ocean idle connection timeout"),
	...
	SESSION_SECRET: Joi.string()
		.default("secrete!session")
		.description("Session secret key"),
	SESSION_LIFETIME: Joi.number()
		.default(7200000)
	...
	CLOUDINARY_URL: Joi.string()
		.uri()
		.required()
		.description("Cloudinary full URL"),
	...
	UNREAL_API_URL_SYNTHESIS: Joi.string()
		.uri()
		.required()
		.description("Unreal API Synthesis URL"),

	PUSH_PUBLIC_KEY: Joi.string()
		.required()
		.description("push notification public key"),
	PUSH_PRIVATE_KEY: Joi.string()
		.required()
		.description("push notification private key"),
}).unknown();
...
const config = {
	...
	mongoose: {
		url: `${envVars.MONGODB_URL}${envVars.DB_NAME}${

...
// Uncomment and import docsRoute if you have it
...
//   },
// ];
defaultRoutes.forEach(({ path, route }: { path: string; route: express.Router }) => {
  Routes.use(path, route);
});
if (config.env === 'development') {
  devRoutes.forEach(({ path, route }: { path: string; route: express.Router }) => {
    Routes.use(path, route);
  });
}

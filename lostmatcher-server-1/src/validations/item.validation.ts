...
const createItem = {
	body: Joi.object().keys({
		...
		contactPersonContact: Joi.string()
			.required()
			.trim()
			.default("no match"),
		status: Joi.string()
...
const uploadItem = {
  body: Joi.object().keys({
    owner: Joi.string().required().custom(objectId).trim(),
    ...
    itemDetails: Joi.object()
      ...
      .required(),
    attributes: Joi.object().optional(), // Optional attributes
  }),
  file: Joi.object().optional(), // Validates file existence
};
